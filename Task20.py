...
стоимость введенного пользователем слова. Будем считать, что на 
...
scrabble = {1: "AEIOULNSTRАВЕИНОРСТ", 2: "DGДКЛМПУ", 3: "BCMPБГЁЬЯ", 
...
for char in word:
    for key, value in scrabble.items():
        if char in value:
            total += key
print(f"Слово {word1} имеет ценность {total}")
